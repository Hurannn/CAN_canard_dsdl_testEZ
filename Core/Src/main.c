...
#define EZ55_BROADCAST_INTERVAL_MS   1000
#define CANARD_MTU         8
...
void send_custom_55_message(void) {
...
int main(void)
{
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
		...
		if ((now - last_55_send_time) >= EZ55_BROADCAST_INTERVAL_MS) {
        send_custom_55_message();
        last_55_send_time = now;
    }
  }
  /* USER CODE END 3 */
}
